...
ip=${msgarray[6]}
ipAddresses="$ip"
logFolder="udp_coded_logs_$numNodes"
...
for (( index=0; index<numNodes; index++ )); do
	ip netns exec ramjet-s1-n$index ./hotstuff-node -numNodes=$numNodes -index=$index -ipAddresses=$ipAddresses &> $logFolder/hotstuff-$index.log &
